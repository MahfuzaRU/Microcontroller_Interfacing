...
/*
1. DATA transmit : after code debug  (transmitter transmit korbe one by one)

step : receiver run -> transmitter run -> transmitter debug

2. data receive :  recervier data receive korbe - asynchronous protocol
step : receiver run -> transmitter run -> receiver debug -> garbage data asle
-> receiver reset -> receiver debug -> transmitted data gulo asbe

if again garbage data ase tahole receiver reset + user mode press kore debug korbo..
tahole transmitted data pabo..
*/